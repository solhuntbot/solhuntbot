...
const CACHE = new NodeCache({ stdTTL: 3600 }); // keep alerted coins for 1 hour
...
⚡ 5-min Tx: ${(token.txns?.m5?.buys || 0) + (token.txns?.m5?.sells || 0)}
...
async function scan() {
  ...
  for (const coin of coins) {
    if (CACHE.has(coin.mint)) continue; // already alerted
    if (!passesFilter(coin)) continue;

    CACHE.set(coin.mint, true); // mark as alerted
    await sendTelegram(coin);
  }
}

// Start scanner
console.log("🚀 Pump.fun New Coin Alpha Bot Running...");
scan();
